...
Endpoints:
  ...
  POST /api/analyze   -> {image: dataURL} -> 5-class probabilities + validated CAM/abstention + uncertainty
...
def health():
    return {
        ...
        "phase": "1.5",
        "features": ["batch", "stress", "validated_cam_or_abstain", "report_release_gates", "xai_embed"],
        "evaluated_scope": "Phase 1 model evaluation on the public Herlev domain only",
...
def report(req: ReportReq):
    ...
    clinical = {
        ...
        "triage": _triage_rule(bethesda, prob, bool(koil)),
    ...
    controlled_explanation = {
        ...
        "scope": "EfficientNet-B0 research output evaluated on the public Herlev domain only.",
        ...
        "limitations": [
            ...
            "KOIL is not validated in the current Phase 1 model.",
        ],
    }
...
def _generate_detailed_llm_explanation(bethesda: str, prob: float, koilocyte: bool, unc: dict) -> dict:
    ...
    base = f"""ผลการคัดกรองโดย AI (CervicalAI – EfficientNet-B0 บนข้อมูลจริง Herlev 1,834 ภาพ) 
...
{f"พบ koilocyte (HPV cytopathic effect) ชัดเจน – บ่งชี้การติดเชื้อ HPV." if koilocyte else ""}
...
**อ้างอิง:**
- Bethesda 2014
- Zhang et al. Sci Rep 2025 (SIPaKMeD 5-class performance 99.22%)
...
{f"พบ koilocyte ร่วมด้วย (อาจเป็น mixed HPV infection)." if koilocyte else ""}
...
- WHO guidelines for invasive cervical cancer management"""
        ...
        detail = base + "พบ koilocyte (HPV effect) – ควรตรวจ HPV genotyping เพิ่มเติม."
...
def report_export(req: ReportExportReq):
    ...
    lines.append(f"KOIL morphology flag: {bool(a.get('koilocyte'))} (unvalidated Phase 2 target)")
...
def demo_script():
    ...
    return {
        ...
        "short": [
            ...
            "2. โชว์ภาพตัวอย่าง → วิเคราะห์ → แสดง 5-class + uncertainty gauge + heatmap",
        ...
        "full": [
            ...
            "ANALYZE: กดวิเคราะห์ → ดู top class + probability bars + koilocyte flag",
...
def _triage_rule(bethesda: str, prob: float, koil: bool, uncertain: bool = False) -> str:
    ...
    if bethesda == "KOIL":
        return "KOIL is not validated in Phase 1; do not use it as a standalone result."
...
def _clinical_action(bethesda: str, koil: bool, uncertain: bool) -> str:
    ...
    if bethesda == "KOIL":
        return "Unvalidated Phase 2 category; require expert review and separate HPV testing if indicated."
...
def _patient_layer(bethesda: str, prob: float, koil: bool, uncertain: bool) -> dict:
    ...
    if bethesda == "KOIL":
        return {
            "result": "No patient result is available for the unvalidated KOIL category.",
            "action": "Obtain expert review and separate testing when clinically indicated.",
            "why": "KOIL is not validated in the current model and does not confirm HPV infection.",
...
def _patient_layer_advanced(bethesda: str, prob: float, koil: bool, uncertain: bool,
    ...
    if bethesda == "LSIL":
        ...
        if koil:
            msg += " + ร่องรอย HPV"
            why += " (HPV เป็นสาเหตุหลักของมะเร็งปากมดลูก)"
            simple = "มีเซลล์ผิดปกติ + อาจมีเชื้อ HPV ต้องตรวจ genotyping"
        return {
            "result": msg,
    ...
    if bethesda == "KOIL":
        return {
            "result": "เห็นเซลล์ที่ถูกไวรัส HPV ทำ (koilocyte)",
            "action": "ตรวจเลือด HPV genotyping (แนะนำ 52/58) + คุมอาการ + ตรวจซ้ำ",
            "why": "HPV บางสายพันธุ์ (โดยเฉพาะ 52/58 ในไทย) เกี่ยวข้องกับมะเร็งปากมดลูก",
            "simple": "พบร่องรอยการติดเชื้อ HPV ในเซลล์ ต้องตรวจเพิ่ม",
            "level": "moderate",
        }
    return {
        "result": "ผลปกติ",
